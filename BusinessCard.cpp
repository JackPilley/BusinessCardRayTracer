...
z;V q+(V v){u V{x+v.x,y+v.y,z+v.z};}V q*(
F a){u V{x*a,y*a,z*a};}F q*(V v){u v.x*x+
v.y*y+v.z*z;}V q^(V n){u *this+(n*(*this*
n)*2)*-1;}F q!(){u x*x+y*y+z*z;}V N(){u*
...
D(){u(F)rand()/RAND_MAX/16-1/32.f;}I main
(){ofstream f("card.ppm",ofstream::binary
);f<<"P6 500 500 255 ";for(I r=0;r<500;r
++){for(I c=0;c<500;c++){F x=-1+c/250.f;F
y=1-r/250.f;V p{x*3,y*3,3};V e{};for(I i=
0;i<60;++i){V o{x+D(),y+D(),1};V d=p+o*-1
;e=e+R(o,d.N());}e=(e*(1/60.f))*255.f;f<<
(B)min(e.x,255.f)<<(B)min(e.y,255.f)<<(B)
min(e.z,255.f);}}u 0;}