...
t=S(o,d,s);if(t>0){V normal=(s+d*t).N();V
p=o+d*t;return V{0,.5,1}*max(normal*l,0.f
)+V{1,1,1}*pow(max(p.N()*(l^normal),0.f),
15)*.6+R(p+(d^normal)*0.01f,d^normal)*.3f
;}if(d.y<0){V p=o+d*(o.y+1.5f)*(1/-d.y);F
t=((I)floor(p.x*2)-(I)floor(p.z*2))&1;
return V{1,t,t}*(S(p,l,V{p.x,p.y-.2f,p.z-
3})>0?.5f:1);}return V{d.y/2+.3f,d.y/2+
.3f,d.y/2+.5f};}F D(){return rand()/(F)
RAND_MAX/16-1/32.f;}I main(){ofstream f(
"c.ppm",ofstream::binary);f<<
"P6 500 500 255\n";for(I r=0;r<500;r++){
...
min(e.x,255.f)<<(B)min(e.y,255.f)<<(B)min
(e.z,255.f);}}return 0;}